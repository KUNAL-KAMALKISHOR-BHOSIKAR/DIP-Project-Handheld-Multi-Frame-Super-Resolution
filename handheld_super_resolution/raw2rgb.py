...
def apply_edge_detection(image, sigma=1.0):
    edges = filters.sobel(image.mean(axis=-1))
    return np.clip(image + edges[..., None], 0, 1)

def apply_bilateral_filter(image, d=9, sigma_color=75, sigma_space=75):
    return cv2.bilateralFilter((image * 255).astype(np.uint8), d, sigma_color, sigma_space) / 255.0

def adjust_hsv(image, hue_shift=0.1, saturation_scale=1.2, brightness_scale=1.1):
    hsv = cv2.cvtColor((image * 255).astype(np.uint8), cv2.COLOR_RGB2HSV)
    h, s, v = cv2.split(hsv)
    h = ((h.astype(np.float32) + hue_shift * 180) % 180).astype(np.uint8)
    s = np.clip(s.astype(np.float32) * saturation_scale, 0, 255).astype(np.uint8)
    v = np.clip(v.astype(np.float32) * brightness_scale, 0, 255).astype(np.uint8)
    hsv = cv2.merge((h, s, v))
    return cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB) / 255.0

def postprocess(raw, img=None, do_color_correction=True, do_tonemapping=True, 
                do_gamma=True, do_sharpening=True, do_devignette=False, xyz2cam=None, sharpening_params=None, do_clahe=False, do_edge_detection=False, do_bilateral=False, do_hsv=None, hsv_params=None):
    """
    ...
    if img is None:
        ...
        ## HSV Adjustment
        if do_hsv:
            img = adjust_hsv(img, **(hsv_params or {}))
        return img
